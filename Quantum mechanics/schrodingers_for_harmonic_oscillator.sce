clc; clear; 
clf();
e = 3.795; hb = 197.3; mu = 0.511D6; m = 940;
E = -13.6; 
k = 100;
b_val = input("Enter the value of a from [0, 10, 30] here :  ")
function p = v(x)
    p = (0.5*((k)*(x*x))) + ((1/3)*((b_val)*(x*x*x)))
endfunction
...
[e_vec,e_val] = spec(B);
disp("The Ground state energy if a = " +string(b_val)+" is : ")

disp("E value is : "+string(hb*hb/(2*m*h*h)*e_val(1,1)) )
subplot(1,2,1)
